...
class UniversalCommandEngine {
  ...
  detectShell() {
    switch (this.platform) {
      case 'win32': return process.env.SHELL || 'powershell.exe';
  ...
  async executeCommand(command, options = {}) {
    ...
    const execution = this.platform === 'win32'
...
class MassiveContextAnalyzer {
  ...
  async analyzeProgressively(context, options) {
    ...
    const analyses = await Promise.all(
      segments.map(segment => this.analyzeSegment(segment, options))
    );
...
class CommandRouter {
  async discoverCommands() {
    const globalCommands = await this.scanDirectory(
      path.join(os.homedir(), '.gemini', 'commands')
    );
    const localCommands = await this.scanDirectory(
