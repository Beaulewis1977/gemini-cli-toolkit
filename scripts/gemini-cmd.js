 ...
 * Wrapper for the context analyzer to integrate with Gemini CLI slash commands
...
class ContextCommand {
  ...
  async execute(mode = 'compact') {
    ...
    try {
      ...
      // Find the current project's .gemini directory
      const projectInfo = await findClaudeDirectory();
      if (!projectInfo) {
        const error = this.SecureErrorHandler.sanitizeError(new Error('No .gemini directory found'));
        console.error(`❌ ${error.error}`);
        console.error('   Make sure you are running this command from within a Gemini CLI project.\n');
      ...
      const globalAnalyzerPath = path.join(os.homedir(), '.gemini', 'scripts', analyzerName);
